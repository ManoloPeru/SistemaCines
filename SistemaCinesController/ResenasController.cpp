#include "ResenasController.h"

using namespace SistemaCinesController;
using namespace System::IO;

ResenasController::ResenasController() {

}

List<Resena^>^ ResenasController::buscarResena(String^ autor) {
...
List<Resena^>^ ResenasController::buscarAll() {
	List<Resena^>^ listaResenasEncontradas = gcnew List<Resena^>();
	array<String^>^ lineas = File::ReadAllLines("resenas.txt");
	String^ separadores = ";";
	for each (String ^ lineaResena in lineas) {
...
void ResenasController::eliminarResena(int codigo) {
...
void ResenasController::escribirArchivo(List<Resena^>^ listaResenas) {
...
void ResenasController::agregarResena(int codigo, String^ autor, String^ correo, String^ descripcion) {
	List<Resena^>^ listaResenas = buscarAll();
...
void ResenasController::actualizarResena(Resena^ objResena) {
	List<Resena^>^ listaResenas = buscarAll();
	for (int i = 0; i < listaResenas->Count; i++) {
...
Resena^ ResenasController::buscarResenaxCodigo(int codigo) {

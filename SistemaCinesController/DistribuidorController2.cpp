#include "DistribuidorController2.h"

using namespace SistemaCinesController;
using namespace System::IO;

DistribuidorController2::DistribuidorController2() {

}

List<Distribuidor^>^ DistribuidorController2::buscarDistribuidor(String^ ruc, String^ contacto, String^ razonSocial) {
...
List<Distribuidor^>^ DistribuidorController2::buscarAll() {
	List<Distribuidor^>^ listaDistribuidoresEncontrados = gcnew List<Distribuidor^>();
	array<String^>^ lineas = File::ReadAllLines("distribuidores.txt");
	String^ separadores = ";";
	for each (String ^ lineaDistribuidor in lineas) {
...
void DistribuidorController2::eliminarDistribuidor(String^ rucDistribuidor) {
...
void DistribuidorController2::escribirArchivo(List<Distribuidor^>^ listaDistribuidor) {
...
void DistribuidorController2::agregarDistribuidor(String^ ruc, int contacto, String^ razonSocial) {
	List<Distribuidor^>^ listaDistribuidor = buscarAll();
...
void DistribuidorController2::actualizarDistribuidor(Distribuidor^ objDistribuidor) {
	List<Distribuidor^>^ listaDistribuidor = buscarAll();
	for (int i = 0; i < listaDistribuidor->Count; i++) {
...
Distribuidor^ DistribuidorController2::buscarDistribuidorxRuc(String^ ruc) {

#include "CinesController2.h"

using namespace SistemaCinesController;
using namespace System::IO;

CinesController2::CinesController2() {

}

List<Cine^>^ CinesController2::buscarCinexIDyDepartamento(int codigo, String^ departamento) {
	{
...
List<Cine^>^ CinesController2::buscarAll() {
	List<Cine^>^ listaCinesEncontrados = gcnew List<Cine^>();
	array<String^>^ lineas = File::ReadAllLines("cines.txt");
	String^ separadores = ";";
	for each (String ^ lineaCine in lineas) {
...
void CinesController2::eliminarCine(int codigo) {
...
void CinesController2::escribirArchivo(List<Cine^>^ listaCines) {
...
void CinesController2::agregarCine(int codigo, String^ departamento, String^ provincia, String^ distrito, String^ direccion, String^ horaInicio, String^ horaFin) {
	List<Cine^>^ listaCines = buscarAll();
...
Cine^ CinesController2::buscarCinexCodigo(int codigo) {
...
void CinesController2::actualizarCine(Cine^ objCine) {
	List<Cine^>^ listaCines = buscarAll(); 

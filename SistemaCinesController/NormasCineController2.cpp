#include "NormasCineController2.h"

using namespace SistemaCinesController;
using namespace System::IO;

NormasCineController2::NormasCineController2() {

}

List<NormasCine^>^ NormasCineController2::buscarNormaCine(String^ tipoNorma) {
...
List<NormasCine^>^ NormasCineController2::buscarAll() {
	List<NormasCine^>^ listaNormasEncontradas = gcnew List<NormasCine^>();
	array<String^>^ lineas = File::ReadAllLines("normasCine.txt");
	String^ separadores = ";";
	for each (String ^ lineaNormaCine in lineas) {
...
void NormasCineController2::eliminarNormaCine(String^ resolucionNormativa) {
...
void NormasCineController2::escribirArchivo(List<NormasCine^>^ listaNormasCine) {
...
void NormasCineController2::agregarNorma(String^ resolucionNormativa, String^ tipoNorma, String^ descripcion, String^ plandeAccion) {
	List<NormasCine^>^ listaNormasCine = buscarAll();
...
void NormasCineController2::actualizarNorma(NormasCine^ objNormasCine) {
	List<NormasCine^>^ listaNormasCine = buscarAll();
	for (int i = 0; i < listaNormasCine->Count; i++) {
...
NormasCine^ NormasCineController2::buscarNormaCinexResolucion(String^ resolucionNormativa) {
